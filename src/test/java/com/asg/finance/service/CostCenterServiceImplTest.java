...
class CostCenterServiceImplTest {
    ...
    void updateCostCenter_ShouldUpdateAndReturnUpdatedCostCenter() {
        // Given
        lenient().when(repository.findById(any())).thenReturn(Optional.of(costCenter));
        lenient().when(repository.existsByCostCenterCodeAndCostCenterPoidNot(any(), any())).thenReturn(false);
        lenient().when(repository.existsByCostCenterDescriptionAndCostCenterPoidNot(any(), any())).thenReturn(false);
        lenient().when(repository.save(any(CostCenter.class))).thenReturn(costCenter);
    ...
    void updateCostCenter_ShouldThrowValidationException_WhenCodeNotUnique() {
        // Given
        costCenter.setCostCenterCode("DIFFERENT_CODE"); // Make existing code different from DTO
        when(repository.findById(any())).thenReturn(Optional.of(costCenter));
