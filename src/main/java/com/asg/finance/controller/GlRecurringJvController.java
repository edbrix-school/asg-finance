...
public class GlRecurringJvController {
        ...
        @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = {
                        @ExampleObject(name = "EMPLOYEE Type Recurring JV", value = """
                                        {
                                        ...
                                        """),
                        @ExampleObject(name = "ASSET Type Recurring JV", value = """
                                        {
                                          "transactionDate": "2025-12-16",
                                          "refType": "ASSET",
                                          "employeePoid": null,
        ...
        public ResponseEntity<?> createRecurringJv(
                ...
                return success("Recurring JV created successfully", recurringJvService.createRecurringJv(request));
        }

        @Operation(summary = "List Recurring JVs")
        @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = @ExampleObject(name = "Recurring JV List", value = """
                        {
                           "operator": "OR",
                           "isDeleted": "N",
                           "filters": [
        ...
        public ResponseEntity<?> listRecurringJvs(
                ...
                Map<String, Object> response = recurringJvService.listRecurringJvs(UserContext.getDocumentId(), filters,
                                startDate, endDate, pageable);
                return success("Recurring JVs list retrieved successfully", response);
        }

        @Operation(summary = "Get recurring JV by ID", description = "Retrieves recurring JV details including header, detail lines, and schedule details", responses = {
                        @ApiResponse(responseCode = "200", description = "Successfully retrieved recurring JV", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RecurringJvResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Recurring JV not found", content = @Content(mediaType = "application/json"))
        }, security = @SecurityRequirement(name = "bearerAuth"))
        @AllowedAction(UserRolesRightsEnum.VIEW)
        @GetMapping("/{transactionPoid}")
        public ResponseEntity<?> getRecurringJvById(
                        @Parameter(description = "Transaction POID of the recurring JV", required = true) @PathVariable Long transactionPoid) {

                loggingService.createLogSummaryEntry(LogDetailsEnum.VIEWED, UserContext.getDocumentId(),
                                transactionPoid.toString());
                return success("Recurring JV retrieved successfully",
                                recurringJvService.getRecurringJvById(transactionPoid));
        }

        @Operation(summary = "Update Recurring JV")
        @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = @ExampleObject(name = "Update Recurring JV", value = """
                        {
                          "transactionDate": "2025-12-16",
                          "refType": "EMPLOYEE",
                          "employeePoid": 1001,
        ...
        public ResponseEntity<?> updateRecurringJv(
                        ...
                        @Valid @RequestBody RecurringJvRequest request) {
                return success("Recurring JV updated successfully", recurringJvService
                                .updateRecurringJv(transactionPoid, request, UserContext.getDocumentId()));
        }
        ...
        @io.swagger.v3.oas.annotations.parameters.RequestBody(content = @Content(examples = @ExampleObject(name = "Create Schedule", value = """
                        {
        ...
        public ResponseEntity<?> createSchedule(
                ...
                return success("Schedule created successfully",
                                recurringJvService.createSchedule(transactionPoid, request));
        }
