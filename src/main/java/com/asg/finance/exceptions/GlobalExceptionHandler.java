    ...
    public ResponseEntity<?> handleDateFormatException(MethodArgumentTypeMismatchException ex) {
        if (ex.getRequiredType() == LocalDate.class) {
            ...
            if (!value.matches("\\d{4}-\\d{2}-\\d{2}"))
                return ApiResponse.badRequest(String.format("Invalid value '%s' for '%s'. Expected format: YYYY-MM-DD.", value, param));
            ...
            try {
                ...
                if (month < 1 || month > 12)
                    return ApiResponse.badRequest(String.format("Invalid month '%d' in '%s'. Must be 01–12.", month, value));

                int maxDay = YearMonth.of(year, month).lengthOfMonth();
                if (day < 1 || day > maxDay)
                    return ApiResponse.badRequest(String.format("Invalid day '%d' for month '%02d' in '%s'. Max days: %d.", day, month, value, maxDay));

                LocalDate.parse(value); // final validation (leap year, etc.)
            } catch (DateTimeException | NumberFormatException e) {
                return ApiResponse.badRequest(String.format("Invalid date '%s'. Please provide a valid calendar date.", value));
            }
