...
class GlChequeCashConvertServiceImplTest {
    ...
    void listOfRecordsAndGenericSearch_returnsWrappedPage() {
        ...
        lenient().when(documentService.resolveFilters(any())).thenAnswer(inv -> ((FilterRequestDto)inv.getArgument(0)).filters());

        when(documentService.search(
                eq("400-110"), anyList(), isNull(), eq(pageable), isNull(),
                eq("TRANSACTION_POID"), eq("DOC_REF")
        )).thenReturn(new RawSearchResult(records, Map.of("TRANSACTION_POID","NUMBER"), 1L));

        Map<String,Object> result = service.listOfRecordsAndGenericSearch("400-110", req, LocalDate.now(), LocalDate.now(), pageable);
